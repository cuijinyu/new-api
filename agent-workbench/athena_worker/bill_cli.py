...
def cmd_bill(args):
    ...
    split_customers = getattr(args, "split_customers", None)
    if split_customers is None:
        split_customers = customer_view and not args.user_id
    bill_type = getattr(args, "bill_type", None)
    split_internal = getattr(args, "split_internal_customers", None)
    if split_internal is None:
        split_internal = bill_type == "internal_customer_bill" and not args.user_id and not args.channel_id
    split_channels = getattr(args, "split_channels", None)
    if split_channels is None:
        split_channels = bill_type == "channel_cost_bill" and not args.channel_id
    result = report_builder.generate_monthly_bill(
        ...
        split_internal_customers=split_internal,
...
def cmd_daily(args):
    ...
    split_channels = getattr(args, "split_channels", None)
    if split_channels is None:
        split_channels = channel_id is None
...
def cmd_kpi(args):
    ...
    df = run_query_cached(
        queries.kpi_summary(month, channel_id=channel_id, user_id=user_id),
        no_cache=args.no_cache,
    )
    if getattr(args, "json", False):
        if df.empty:
            ...
            kpi = df.iloc[0]
            payload = {
                "total_usd": float(kpi["total_usd"]),
                "total_calls": int(kpi["total_calls"]),
                "unique_users": int(kpi["unique_users"]),
                "unique_models": int(kpi["unique_models"]),
                "total_input_tokens": int(kpi["total_input_tokens"]),
                "total_output_tokens": int(kpi["total_output_tokens"]),
            }
        import json
...
def build_parser():
    ...
    p_bill.add_argument("--split-customers", dest="split_customers", action="store_true", default=None,
                        help="客户版汇总账单之外，为每个客户再生成独立账单（默认：--customer-view 且未指定 --user-id 时启用）")
    p_bill.add_argument("--no-split-customers", dest="split_customers", action="store_false",
                        help="仅生成汇总客户版账单，不拆分各客户")
    p_bill.add_argument("--bill-type", choices=["customer_invoice", "internal_customer_bill", "channel_cost_bill"],
                        help="账单类型（影响默认拆分行为）")
    p_bill.add_argument("--split-internal-customers", dest="split_internal_customers", action="store_true", default=None,
                        help="内部版汇总账单之外，为每个客户再生成独立账单")
    p_bill.add_argument("--no-split-internal-customers", dest="split_internal_customers", action="store_false",
                        help="仅生成汇总内部版账单，不拆分各客户")
    p_bill.add_argument("--split-channels", dest="split_channels", action="store_true", default=None,
                        help="渠道汇总账单之外，为每个渠道再生成独立账单")
    p_bill.add_argument("--no-split-channels", dest="split_channels", action="store_false",
                        help="仅生成汇总渠道账单，不拆分各渠道")
    ...
    p_daily.add_argument("--channel-id", type=int, help="指定上游渠道 ID")
    p_daily.add_argument("--split-channels", dest="split_channels", action="store_true", default=None,
                         help="汇总日报之外，为每个渠道再生成独立快照（默认启用）")
    p_daily.add_argument("--no-split-channels", dest="split_channels", action="store_false",
                         help="仅生成汇总日报，不拆分各渠道")
    p_daily.add_argument("--detail", action="store_true", help="导出当日逐条明细")
    ...
    p_kpi.add_argument("--json", action="store_true", help="输出 JSON（供 API 调用）")
    p_kpi.add_argument("--channel-id", type=int, help="指定上游渠道 ID")
    p_kpi.add_argument("--user-id", type=int, help="指定用户 ID")
