...
def _load_discounts() -> dict:
    ...
    if "rules" in _discounts_cache and "cost_discounts" not in _discounts_cache:
        converted = {
            "_version": _discounts_cache.get("_version", "fixture"),
            "cost_discounts": {"defaults": {"*": 1.0}, "by_channel": {}},
            "revenue_discounts": {"defaults": {"*": 1.0}, "by_user": {}},
        }
        for rule in _discounts_cache.get("rules", []):
            pattern = rule.get("model_pattern") or "*"
            multiplier = float((rule.get("discount_value_json") or {}).get("multiplier", 1.0))
            if rule.get("scope_type") == "channel" and rule.get("channel_id") is not None:
                converted["cost_discounts"]["by_channel"].setdefault(str(rule["channel_id"]), {})[pattern] = multiplier
            if rule.get("scope_type") == "user" and rule.get("user_id") is not None:
                converted["revenue_discounts"]["by_user"].setdefault(str(rule["user_id"]), {})[pattern] = multiplier
        _discounts_cache = converted
            ...
            "models": [
                {
                    ...
                    "type": "flat",
                    "flat_tier": False,
                    "ip": float(value.get("input", 0)),
                    "op": float(value.get("output", 0)),
                    "chp": 0.0,
                    "cwp": float(value.get("input", 0)),
                    "cwp_1h": float(value.get("input", 0)),
                }
