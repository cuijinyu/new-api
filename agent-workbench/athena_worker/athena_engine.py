...
def run_query_cached(sql: str, ttl: int | None = ..., no_cache: bool = False) -> pd.DataFrame:
    ...
    """
    if os.getenv("ATHENA_E2E_MODE", "").lower() == "fixture":
        return _fixture_query(sql)
...
def _fixture_dir() -> str:
    return os.getenv("ATHENA_FIXTURE_DIR", "/workspace/e2e/fixtures/athena")


def _extract_year_month(sql: str) -> str:
    match = re.search(r"year\s*=\s*'(\d{4})'.{0,80}?month\s*=\s*'(\d{2})'", sql, re.IGNORECASE | re.DOTALL)
    if match:
        return f"{match.group(1)}-{match.group(2)}"
    match = re.search(r"(\d{4})-(\d{2})-\d{2}", sql)
    if match:
        return f"{match.group(1)}-{match.group(2)}"
    return os.getenv("WORKBENCH_E2E_MONTH") or "2026-06"


def _extract_channel_id(sql: str) -> int | None:
    match = re.search(r"\bchannel_id\s*=\s*(\d+)", sql, re.IGNORECASE)
    return int(match.group(1)) if match else None


def _extract_user_id(sql: str) -> int | None:
    match = re.search(r"\buser_id\s*=\s*(\d+)", sql, re.IGNORECASE)
    return int(match.group(1)) if match else None


def _fixture_usage(sql: str) -> pd.DataFrame:
    year_month = _extract_year_month(sql) or "2026-06"
    path = os.path.join(_fixture_dir(), f"usage_logs_{year_month.replace('-', '_')}.csv")
    df = pd.read_csv(path)
    df["day_num"] = pd.to_datetime(df["day"]).dt.day
    df["hour"] = 0
    df["username"] = df["user_id"].apply(lambda value: f"user_{int(value)}")
    df["model_name"] = df["model"]
    df["token_name"] = df["model_name"]
    df["prompt_tokens"] = df["input_tokens"]
    df["completion_tokens"] = df["output_tokens"]
    df["total_tokens"] = df["prompt_tokens"] + df["completion_tokens"]
    df["cache_read_tokens"] = df.get("cache_read_tokens", 0)
    df["cache_write_tokens"] = df.get("cache_write_tokens", 0)
    df["total_quota"] = (df["total_tokens"] + df["cache_read_tokens"] + df["cache_write_tokens"]).astype(int)
    df["total_usd"] = (df["total_quota"] / QUOTA_TO_USD).round(4)
    channel_id = _extract_channel_id(sql)
    if channel_id is not None:
        df = df[df["channel_id"].astype(int) == channel_id]
    user_id = _extract_user_id(sql)
    if user_id is not None:
        df = df[df["user_id"].astype(int) == user_id]
    return df


def _fixture_group(df: pd.DataFrame, keys: list[str]) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame()
    grouped = (
        df.groupby(keys, dropna=False)
        .agg(
            call_count=("request_id", "nunique"),
            total_input_tokens=("prompt_tokens", "sum"),
            total_output_tokens=("completion_tokens", "sum"),
            total_tokens=("total_tokens", "sum"),
            total_cache_hit_tokens=("cache_read_tokens", "sum"),
            total_cache_write_tokens=("cache_write_tokens", "sum"),
            total_cw_5m=("cache_write_tokens", "sum"),
            total_cw_1h=("cache_write_tokens", lambda _: 0),
            total_cw_remaining=("cache_write_tokens", lambda _: 0),
            total_quota=("total_quota", "sum"),
            total_usd=("total_usd", "sum"),
        )
        .reset_index()
    )
    grouped["total_usd"] = grouped["total_usd"].astype(float).round(4)
    grouped["total_image_output_tokens"] = 0
    grouped["total_image_input_tokens"] = 0
    return grouped


def _fixture_query(sql: str) -> pd.DataFrame:
    df = _fixture_usage(sql)
    normalized = " ".join(sql.lower().split())

    if "count(*) as total_calls" in normalized:
        return pd.DataFrame(
            [
                {
                    "total_calls": int(df["request_id"].nunique()),
                    "unique_users": int(df["user_id"].nunique()),
                    "unique_models": int(df["model_name"].nunique()),
                    "total_input_tokens": int(df["prompt_tokens"].sum()),
                    "total_output_tokens": int(df["completion_tokens"].sum()),
                    "total_quota": int(df["total_quota"].sum()),
                    "total_usd": round(float(df["total_usd"].sum()), 2),
                }
            ]
        )

    if "group by hour" in normalized:
        result = _fixture_group(df, ["hour"])
        return result[["hour", "call_count", "total_tokens", "total_usd"]]

    if "group by day, token_name, model_name" in normalized:
        result = _fixture_group(df, ["day_num", "token_name", "model_name"])
        result = result.rename(columns={"day_num": "day"})
        return result.sort_values(["day", "token_name", "total_usd"], ascending=[True, True, False])

    if "group by day" in normalized:
        result = _fixture_group(df, ["day_num"])
        result = result.rename(columns={"day_num": "day"})
        return result.sort_values("day")

    if "group by user_id, username, channel_id, model_name" in normalized:
        result = _fixture_group(df, ["user_id", "username", "channel_id", "model_name"])
        return result.sort_values(["user_id", "total_usd"], ascending=[True, False])

    if "group by user_id, username, model_name" in normalized:
        result = _fixture_group(df, ["user_id", "username", "model_name"])
        return result.sort_values(["user_id", "total_usd"], ascending=[True, False])

    if "group by user_id, username" in normalized:
        result = _fixture_group(df, ["user_id", "username"])
        return result.sort_values("total_usd", ascending=False)

    if "group by model_name" in normalized:
        result = _fixture_group(df, ["model_name"])
        result["avg_latency_sec"] = 1.0
        result["stream_pct"] = 0.0
        return result[["model_name", "call_count", "total_tokens", "total_usd", "avg_latency_sec", "stream_pct"]].sort_values(
            "total_usd", ascending=False
        )

    if "group by channel_id" in normalized:
        result = _fixture_group(df, ["channel_id"])
        result["model_count"] = int(df["model_name"].nunique())
        result["user_count"] = int(df["user_id"].nunique())
        return result[["channel_id", "call_count", "model_count", "user_count", "total_usd"]].sort_values(
            "total_usd", ascending=False
        )
