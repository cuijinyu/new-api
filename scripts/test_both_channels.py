...
# -*- coding: utf-8 -*-
"""
分别测试两个渠道的缓存命中率 + 代理亲和路由对比
A: 直接调用 aws5101 上游
B: 直接调用 fineapi(anyfast) 上游
C: 通过代理 + 亲和路由
"""
...
def send(url, key, payload):
    ...
    try:
        ...
        return f"FAIL({e})", 0, 0, 0


def run_test(label, url, key, n=10, delay=1.5):
    print(f"\n{'='*70}")
    print(f"  {label}")
    print(f"  URL: {url}")
    ...
    errors = sum(1 for r in results if r.startswith("ERROR") or r.startswith("FAIL"))
...
if __name__ == "__main__":
    ...
    r_aws = run_test("A: 直接调用 aws5101 上游", AWS_URL, AWS_KEY, N, DELAY)
    r_fine = run_test("B: 直接调用 fineapi(anyfast) 上游", FINE_URL, FINE_KEY, N, DELAY)
    r_proxy = run_test("C: 通过代理 + 亲和路由", PROXY_URL, PROXY_KEY, N, DELAY)
    ...
    for label, (hits, creates, errors, total) in [
        ...
        print(f"  {label:20s}  HIT={hits:2d}/{total}  ({pct:3.0f}%)  [{bar}]")

    print()
    best = max(r_aws[0], r_fine[0], r_proxy[0])
    if r_proxy[0] >= best:
        print("  --> 代理+亲和路由 达到或超过直接调用的缓存命中率!")
    elif r_proxy[0] > 0:
        print(f"  --> 代理+亲和路由 有 {r_proxy[0]} 次缓存命中 (vs 直接 aws={r_aws[0]}, fineapi={r_fine[0]})")
    else:
        print("  --> 代理+亲和路由 未命中缓存，上游内部负载均衡是主要瓶颈")
