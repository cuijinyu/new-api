...
# -*- coding: utf-8 -*-
"""
测试 metadata.user_id 对上游 Claude 缓存节点亲和性的影响

上游 MAAS 供应商可能基于 metadata.user_id 做内部路由亲和，
将同一 user_id 的请求路由到同一 Claude 后端节点，从而提高缓存命中率。

测试组：
A: 直接调用 aws5101 + 同一 user_id（验证上游是否支持 metadata 亲和）
B: 直接调用 aws5101 + 不传 user_id（对照组）
C: 通过代理+亲和路由 + 同一 user_id（我们的亲和路由 + 上游 metadata 亲和）
D: 通过代理+亲和路由 + 不传 user_id（只有我们的亲和路由）
E: 直接调用 fineapi + 同一 user_id（验证 fineapi 是否支持 metadata 亲和）
F: 直接调用 fineapi + 不传 user_id（对照组）
"""
...
请基于以上专业知识为用户提供准确详细有深度的技术解答。你的目标是成为用户最可靠的技术顾问，帮助他们解决复杂的技术问题，提升技术能力。无论问题多么简单或复杂，你都应该给出专业、准确、有价值的回答。""" * 2


def build_payload(prompt, user_id=None):
    payload = {
...
def run_group(label, url, key, user_id, n=8, delay=5):
    tid = uuid.uuid4().hex[:8]
    prompt = f"[SESSION:{tid}]\n\n{BASE_PROMPT}"
    uid_desc = f'"{user_id}"' if user_id else "不传"

    print(f"\n{'='*72}")
    print(f"  {label}")
    print(f"  URL: {url}")
    print(f"  metadata.user_id: {uid_desc}")
    print(f"  {n} 次请求, 间隔 {delay}s")
    print(f"{'='*72}")

    results = []
    for i in range(n):
        payload = build_payload(prompt, user_id)
...
if __name__ == "__main__":
    ...
    DELAY = 5
    print("=" * 72)
    print("  metadata.user_id 缓存亲和性对比测试")
    print(f"  时间: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  每组 {N} 次, 间隔 {DELAY}s")
    print("=" * 72)

    all_results = {}

    all_results["A: aws+uid"]    = run_group("A: aws5101 + user_id=testUser1",    AWS_URL,   AWS_KEY,   "testUser1", N, DELAY)
    all_results["B: aws 无uid"]  = run_group("B: aws5101 + 不传 user_id",          AWS_URL,   AWS_KEY,   None,         N, DELAY)
    all_results["C: fine+uid"]   = run_group("C: fineapi + user_id=testUser1",     FINE_URL,  FINE_KEY,  "testUser1", N, DELAY)
    all_results["D: fine 无uid"] = run_group("D: fineapi + 不传 user_id",           FINE_URL,  FINE_KEY,  None,         N, DELAY)
    all_results["E: 代理+uid"]   = run_group("E: 代理+亲和 + user_id=testUser1",    PROXY_URL, PROXY_KEY, "testUser1", N, DELAY)
    all_results["F: 代理 无uid"] = run_group("F: 代理+亲和 + 不传 user_id",          PROXY_URL, PROXY_KEY, None,         N, DELAY)

    print(f"\n\n{'='*72}")
    print(f"  汇总对比")
    print(f"{'='*72}")
    print(f"  {'测试':<18} {'命中/总数':<12} {'命中率':<10}")
    print(f"  {'-'*42}")
    for k, (h, t) in all_results.items():
        rate = f"{h/t*100:.0f}%"
        bar = "*" * h + "." * (t - h)
        print(f"  {k:<18} {h}/{t:<10} {rate:<10} [{bar}]")

    print(f"\n  分析:")
    a_h = all_results["A: aws+uid"][0]
    b_h = all_results["B: aws 无uid"][0]
    c_h = all_results["C: fine+uid"][0]
    d_h = all_results["D: fine 无uid"][0]
    e_h = all_results["E: 代理+uid"][0]
    f_h = all_results["F: 代理 无uid"][0]

    if a_h > b_h:
        print(f"  - aws5101: user_id 提升了缓存命中 ({a_h} vs {b_h}), 上游支持 metadata 亲和!")
    elif a_h == b_h:
        print(f"  - aws5101: user_id 无明显影响 ({a_h} vs {b_h})")
    else:
        print(f"  - aws5101: 不传 user_id 反而更好 ({b_h} vs {a_h})")

    if c_h > d_h:
        print(f"  - fineapi: user_id 提升了缓存命中 ({c_h} vs {d_h}), 上游支持 metadata 亲和!")
    elif c_h == d_h:
        print(f"  - fineapi: user_id 无明显影响 ({c_h} vs {d_h})")
    else:
        print(f"  - fineapi: 不传 user_id 反而更好 ({d_h} vs {c_h})")

    if e_h > f_h:
        print(f"  - 代理+亲和: user_id 进一步提升 ({e_h} vs {f_h})")
    elif e_h == f_h:
        print(f"  - 代理+亲和: user_id 无额外影响 ({e_h} vs {f_h})")
