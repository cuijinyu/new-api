#!/usr/bin/env node
/**
 * S3 错误日志高速并发下载器 —— 将指定日期（可选时间段）的错误请求原文批量下载到本地缓存目录。
 * 错误日志由 Go 服务在上游返回非 2xx 时写入独立的 S3 prefix（默认 llm-error-logs）。
 *
 * 用法:
 *   node s3_download_errors.mjs --date 2026-03-19
 *   node s3_download_errors.mjs --date 2026-03-19 --hour-range 08 14
 *   node s3_download_errors.mjs --date-range 2026-03-01 2026-03-10
 *   node s3_download_errors.mjs --date 2026-03-19 --concurrency 300
 */
...
function parseCli() {
  const { values } = parseArgs({
    options: {
      ...
      "hour-range": { type: "string", multiple: true },
      ...
      prefix:       { type: "string", default: env("ERROR_LOG_S3_PREFIX", env("S3_ERROR_PREFIX", "llm-error-logs")) },
...
function parseHourRange(args) {
  if (!args["hour-range"] || args["hour-range"].length < 2) return null;
  const [startStr, endStr] = args["hour-range"];
  const start = parseInt(startStr, 10);
  const end = parseInt(endStr, 10);
  if (isNaN(start) || isNaN(end) || start < 0 || end > 23 || start > end) {
    console.error(`错误: --hour-range 应为 0-23 的整数区间，如 --hour-range 08 14`);
    process.exit(1);
  }
  return { start, end };
}

function buildS3Prefixes(basePrefix, dateStr, hourRange) {
  ...
  if (!hourRange) {
    return [`${basePrefix}/${y}/${m}/${d}/`];
  }

  const prefixes = [];
  for (let h = hourRange.start; h <= hourRange.end; h++) {
    prefixes.push(`${basePrefix}/${y}/${m}/${d}/${String(h).padStart(2, "0")}/`);
  }
  return prefixes;
}
...
async function main() {
  ...
  const hourRange = parseHourRange(args);
  ...
  console.log(`  S3 错误日志下载器 (Node.js)`);
  console.log(`  桶: ${bucket}, 区域: ${region}, 前缀: ${prefix}, 并发: ${concurrency}`);
  console.log(`  缓存目录: ${cacheDir}`);
  console.log(`  日期: ${dates.join(", ")}`);
  if (hourRange) {
    console.log(`  时间段: ${String(hourRange.start).padStart(2, "0")}:00 - ${String(hourRange.end).padStart(2, "0")}:59 (UTC)`);
  }

  for (const dateStr of dates) {
    const s3Prefixes = buildS3Prefixes(prefix, dateStr, hourRange);

    let allKeys = [];
    for (const s3Prefix of s3Prefixes) {
      const label = hourRange ? s3Prefix : `${prefix}/${dateStr.replace(/-/g, "/")}`;
      process.stderr.write(`  [${dateStr}] 列举 ${label} ...\n`);
      const keys = await listAllKeys(client, bucket, s3Prefix);
      allKeys = allKeys.concat(keys);
    }

    console.log(`  [${dateStr}] 找到 ${allKeys.length} 个错误日志文件`);
    if (allKeys.length === 0) continue;

    const uncached = allKeys.filter(k => !existsSync(join(cacheDir, k)));
    const cachedCount = allKeys.length - uncached.length;
