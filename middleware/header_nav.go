...
func parseHeaderNavAccess(raw any, fallback headerNavAccess) headerNavAccess {
	...
	case bool:
		return headerNavAccess{Enabled: value, RequireAuth: fallback.RequireAuth}
	case string:
		return headerNavAccess{Enabled: parseHeaderNavBool(value, fallback.Enabled), RequireAuth: fallback.RequireAuth}
	case float64:
		return headerNavAccess{Enabled: parseHeaderNavBool(value, fallback.Enabled), RequireAuth: fallback.RequireAuth}
	case map[string]any:
		access := fallback
