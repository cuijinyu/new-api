...
| contents[].parts | array[object] | 是 | 内容分段 |
| contents[].parts[].text | string | 否 | 文本提示词 |
| generationConfig | object | 否 | 生成配置选项 |
| generationConfig.responseModalities | array[string] | 否 | 响应模态 (TEXT, IMAGE) |
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| candidates | array[object] | 生成结果列表 |
| candidates[].content | object | 内容对象 |
| candidates[].content.parts | array[object] | 内容分段，包含文本或生成的图像数据 |
