...
1. **文本长度限制**：单次请求最大支持 10000 字符
2. **计费方式**：按调用次数计费，每次 0.05 元，与文本长度无关
3. **同步接口**：该接口为同步接口，直接返回生成的音频信息，无需轮询任务状态
4. **音频格式**：返回的音频为 MP3 格式
5. **音色选择**：请确保使用有效的 `voice_id`，可从可灵平台获取可用音色列表
